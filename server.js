...
app.get("/code", (request, response) =>
 ...
 app.get("/generative", (request, response) =>
  {
      response.sendFile(__dirname + "/src/generative.html");
  });
...
app.post('/generateSimulatedPrompt', async (req, res) => {
  ...
  try {
        ...
        res.json({ embedTags, explanatoryText });
      } catch (error) {
...
app.post('/generatePrompt', async (req, res) => {
  ...
  console.log(prompt2Send);
    ...
    try {
      ...
      res.status(500).json({ error: 'Prompt generation Error.' });
    }
