(async () => {
    ...
    sendPrompt.addEventListener("click", async function () {
        ...
        try{
            ...
            const { embedTags, explanatoryText } = await response.json();
            ...
            divEmbed.innerHTML = marked.parse(explanatoryText);
            divElement.appendChild(divEmbed);
            embedsContainer.appendChild(divElement);
    ...
    sendButton.addEventListener("click", async function () {
        ...
        try{
            const response =  await fetch("generateSimulatedPrompt", {
                method: 'POST'
                ,headers:{ 'Content-Type': 'application/json' }
                ,body: JSON.stringify({ userPrompt })
            });
